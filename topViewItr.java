...
class topViewItr{
    ...
    public static int[] topViewItr(Node node){
        width(node, 0);
        ...
        while(q.size()>0){
            ...
            while(sz-- >0){
                Pair rp = q.remove();
                int val = rp.state;
                if(ans[val-leftMin] == Integer.MAX_VALUE){
    ...
    public static void main(String[] args){
        ...
        int[] ans = topViewItr(root);
        System.out.print(ans[-leftMin] + " ");
        int left = -leftMin -1;
        int right = -leftMin+1;
        ...
        while(left>=0){
            System.out.print(ans[left] + " ");
            left--;
        }

        while(right<ans.length){
            System.out.print(ans[right] + " ");
            right++;
        }
